...
def test_full_pipeline(mock_log_query, mock_inference, mock_hybrid_search):
    ...
    mock_point.payload = {
        ...
        "section_number": "10",
        "chunk_id": "10_chunk_1",
        "text": "Agricultural income shall not be included in total income."
    }
    mock_point.score = 0.95
    mock_hybrid_search.return_value = [mock_point]
    
    # Mock LLM
    mock_inference.return_value = "Agricultural income is exempt from tax under Section 10."
    
    # Mock DB logging
    mock_log_query.return_value = 1
    
    # Run pipeline
    response = run_pipeline("Is agricultural income taxable?", log_to_db=True)
    
    assert response.question == "Is agricultural income taxable?"
    assert response.answer == "Agricultural income is exempt from tax under Section 10."
